...
const getImagenes = async (req, res) => {
  try {
    ...
    if (!findImages) {
      return res
        .status(204)
        .json({ success: true, message: "Image collection is empty" });
    }

    const imagesWithBase64 = await Promise.all(
      findImages.map(async (image) => {
        const imageBuffer = await admin
          .storage()
          .bucket("gs://fabrica-de-musculos-server.appspot.com")
          .file(image.path)
          .download();
...
const createImagen = async (req, res) => {
  ...
  if (!name || !message) {
    return res
      ...
      .json({
        success: false,
        message: "Image creation failed, fields are empty",
      });
  }
  ...
  try {
    ...
    if (!file) {
      return res
        ...
        .json({
          success: false,
          message: "Image creation failed, no image file provided",
        });
    }
