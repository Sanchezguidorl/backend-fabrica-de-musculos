...
const getButtons = async (req, res) => {
  try {
    ...
    if (!buttons)
      return res
        .status(204)
        .json({ success: true, message: "Collection is empty" });

    return res.status(200).json(buttons);
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Error when entering button data" });
  }
};
...
const createButton = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ success: true, message: "Button successfully added" });
  } catch (error) {
    return res
      .status(500)
      .json({ success: false, message: "Button creation failed" });
  }
};
...
const deleteButton = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Image was deleted" });
  } catch (error) {
    return res
      .status(500)
      .json({
        success: false,
        message: "Failed button deletion",
        error: error.message,
      });
  }
};

module.exports = { getButtons, createButton, deleteButton };
